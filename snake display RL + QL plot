...
class SnakeGame():
    ...
    def checkValid(self, direction):
        ...
        if newY == -1 or newY == self.height // self.cell_size:
            return False
        if self.board[newY, newX] in (self.bodyVal, self.obstacleVal):
            return False
        return True
    ...
    def plottableBoard(self):
        ...
        board = np.zeros([self.width // self.cell_size, self.height // self.cell_size])
    ...
    def evaluateScore(self, numRuns):
        # Evaluate the average score of the snake over multiple runs
        scores = []
        for i in range(numRuns):
            ...
            while not game_over:
                action = self.chooseAction()
                _, reward, game_over, score = self.makeMove(action)
            scores.append(score)
    ...
    def train(self, numEpisodes):
        ...
        for episode in range(numEpisodes):
            ...
            while not game_over:
                action = self.chooseAction()
                new_state, reward, game_over, score = self.makeMove(action)
...
if __name__ == "__main__":
    ...
    game.train(2000)  # Train for 1000 episodes
